...
//#define kCalendarBasicColor [UIColor blueColor] //[UIColor colorWithRed:252.0 / 255.0 green:60.0 / 255.0 blue:60.0 / 255.0 alpha:1.0]
...
- (void)setupCollectionViews {
    ...
    flowLayout.itemSize = CGSizeMake(self.bounds.size.width / 7.01, self.bounds.size.width / 7.0 * 0.85);
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    cell.todayCircle.backgroundColor = NAVIGATIONBAR_COLOR;
        ...
        if (indexPath.row >= firstWeekday && indexPath.row < firstWeekday + totalDays) {
            ...
            if ((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear])) {
                if (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1) {
                    ...
                    // 当天之后
                    cell.todayCircle.backgroundColor = [UIColor clearColor];
                    cell.todayLabel.textColor = [UIColor colorWithWhite:0.0 alpha:1.0];
//                    cell.userInteractionEnabled = NO;
                }
            } else if ((monthInfo.month < [[NSDate date] dateMonth]) || (monthInfo.year < [[NSDate date] dateYear])) {
                //  小于当前月的日子
                cell.todayLabel.textColor = [UIColor colorWithWhite:0.85 alpha:1.0];
                cell.todayCircle.backgroundColor = [UIColor clearColor];
                cell.userInteractionEnabled = NO;
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    self.selectedCell.todayCircle.backgroundColor = [UIColor whiteColor];
    self.selectedCell.todayLabel.textColor = [UIColor blackColor];
